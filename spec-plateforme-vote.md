...
| `state` | enum | `draft` \| `announced` \| `open` \| `closed` \| `published` \| `withdrawn`; `announced` is an optional waypoint (R-3.10, §4, §6.6), `withdrawn` a terminal one reachable only from the four public states (R-3.11, §4, §6.6) |
...
`draft → [announced] → open → closed → published`, irreversible (R-3.2). `announced` is an optional waypoint (R-3.10): a poll may also go straight `draft → open`, exactly as if the state did not exist. A fifth, terminal state, `withdrawn`, branches off `announced`, `open`, `closed` or `published` — and only off those four (R-3.11) — and joins nothing: like `published`, no transition leaves it.
