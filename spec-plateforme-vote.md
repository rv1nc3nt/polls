...
**Storage is content-addressed.** `OptionImage.file`'s path is named after the SHA-256 digest of the bytes, under the referenced option's own id — never after the upload's own filename. This is what keeps the R-3.10 guarantee ("the public page cannot change under a viewer's eyes") extending to an image the way it already does to text: re-uploading edited bytes for what an operator thinks of as "the same picture" produces a new digest, a new path and a new row, so it can never land at the path a frozen `details` reference already points to. Re-uploading *identical* bytes for the same option is a no-op, returning the existing row rather than a duplicate (`(option_id, content_hash)` unique).

**Frozen alongside the option.** `details` sits on the same `PollOption` row as `label`, so the *existing* `inv6_option_update_frozen`/`inv6_option_insert_frozen` triggers of §5 already cover it — no trigger change for that field. `OptionImage` is a separate table with no `poll` column of its own, so it needs its own three triggers, reading the poll's state through `option_id`: no image may be inserted, updated or deleted once the owning poll is not `draft`. An image can therefore only ever be attached to, or removed from, an option while the poll — and the option, and its label — are all still freely editable; there is no path to add or swap an image once a poll has left `draft`, exactly as there is none to add or correct a translation (§3.8).
...
- **INV-6** Poll configuration fields other than `closes_at` are immutable once `state ≠ draft`. Extends to `PollOption.details` (the same row as `label`, no separate trigger needed) and to `OptionImage` (its own three triggers, reading the state through `option_id` — §3.1 bis, R-3.12).
...
| T-79 | An option's `details` rendered with: a YouTube fence carrying a valid id; one carrying `"; DROP TABLE elections_poll; --"` in place of an id; a bare `https://youtu.be/<id>` link alone on its own line; that same link written as `[texte](url)` and again mid-sentence; raw `<script>` and `<iframe>` tags typed directly into the Markdown; an `image:<uuid>` reference to an image that belongs to a *different* option; and one to an id that does not exist at all | The valid fence and the bare link each become an `<iframe>` in the output, both pointed at `youtube-nocookie.com`; the invalid id drops the whole block with no iframe and no error; the `[texte](url)` form and the mid-sentence mention stay ordinary links, not embeds; the typed `<script>`/`<iframe>` are stripped by the sanitiser and never reach the page; both foreign and non-existent image references are dropped silently, leaving the rest of the text intact (§3.1 bis, R-3.12) |
| T-80 | `INSERT`/`UPDATE`/`DELETE` on `elections_optionimage` attempted in raw SQL against an option of a poll that is `open`; the same three against a poll still `draft`; and an image re-uploaded with byte-identical content to one already attached to the same option | The three against the `open` poll are all rejected by the `inv6_optionimage_*_frozen` triggers; the same three against the `draft` poll succeed; the identical re-upload returns the existing row rather than creating a duplicate, and a second, distinct image re-uploaded under the same option succeeds as a separate row (§3.1 bis, R-3.12) |
