...
- `draft → announced` may be triggered manually at any time the guards above allow (translations complete, `opens_at` still in the future). `announced → open` may be triggered manually **at any time**, including ahead of `opens_at`, in which case **`opens_at` is set to the instant of opening** (R-3.4). The write paths depend only on the clock against `opens_at` (above), never on `state`, so leaving `opens_at` at its configured value would have produced a poll that reads `open` in the back-office while the public site — which follows the clock — still says it has not started, and no ballot or registration would be accepted. Moving it makes the poll votable and listed from the moment the poll admin clicks. It is the only way `opens_at` ever changes after `draft`, and only earlier: the audit event of the transition carries the previous and the new value in `before`/`after`. The scheduled `open_poll` selects on `opens_at ≤ now` and never moves it.
...
**The voting window (INV-2)** is checked in one service function that every ballot write path calls, taking the poll and the current time, and rejecting a write outside the window for that ballot's source — before `opens_at`, at or after `closes_at` for an online ballot, at or after `paper_entry_deadline` for a paper one. It does not consult `state`, since the scheduled transition may not have run yet (§4) — with one exception: `withdrawn` (R-3.11) is never a scheduled transition, only a deliberate one-off action, so the window function refuses unconditionally the instant a poll reaches it, before it even looks at the clock. This is the invariant the typestate approach could not have delivered either, since elapsing time is not a method call; here as there, the clock is consulted at the point of use. The retention purge is the one writer permitted past `closes_at`; it goes through a separate, named service function of which it is the only caller. The INV-2 trigger carries the same `withdrawn` exception on `INSERT` and `UPDATE`, an exact mirror of the `DELETE` carve-out §11 describes for the retention purge.
