...
| `options` | ordered list of `{id, label, details}` | ≥ 2; `details` is §3.1 bis's optional extended description (R-3.12) |
...
| `preview_token` | text, blank by default | R-3.10 bis's share link; generated/regenerated/revoked only while `draft`, or in any state where `is_sandbox` (R-3.7, §6.6); a lifecycle/access field, deliberately outside `FROZEN_CONFIG_FIELDS` |
...
### 3.1 bis Markdown descriptions and `PollImage` (R-3.12)

Two fields hold raw Markdown rather than rendered HTML: `Poll.description` (R-3.1's field, unchanged in shape) and `PollOption`'s `details`, a `{language_code: markdown}` map exactly like `label_i18n` (§3.8). `PollImage` is a new table: `{id, poll_id, short_id, file, content_type, content_hash, alt_text, uploaded_at}`, one small library per poll rather than one per option — a row is usable from the poll's own `description` and from *any* of its options' `details`, uploaded once regardless of how many descriptions reference it. `short_id` is a sequential integer per poll, assigned in upload order by the service layer rather than left as the row's own UUID, since an operator typing `image:3` into a description has no row open in front of them to copy a UUID from.

**The option's `details` is optional, and never a translation gate**; the poll's own `description` is not — it keeps the same translation-gate rule R-14.3 already gives it. Unlike the title, the description and the label, a missing `details` translation is not among `missing_translations()`'s gaps and never blocks `announce_poll` or `open_poll` (R-3.10, R-14.3, §3.8): an enabled language with no extended description falls back to the poll's default-language one exactly as `Poll.translate` already does for every other field, or renders nothing where that is itself absent. Absent content here is a normal outcome, not a gap to report.

**Rendered at display time, never at save time.** A fix to the allowed-tag set below, or to the YouTube pattern, then reaches every poll's content immediately — past and present — with no backfill migration. Rendering is a pure function of `(poll, raw markdown)`, shared by both fields behind two entry points — `render_poll_description(poll, language)` and `render_option_details(option, language)` — with three steps, run in this order because each closes off a category of untrusted input before the next is given a chance to reopen it:

1. **Images.** `![alt](image:<n>)` is resolved to the image's real, storage-backed URL before the text reaches the Markdown parser at all, and only when `<n>` names a `PollImage` belonging to *this* poll — a reference to another poll's image id, or to one that does not exist, is silently dropped rather than followed. There is no bare-URL image syntax: every image a description shows was uploaded through screen 2, never fetched from an address the operator merely typed (§14's self-hosting posture: the platform is not a means of embedding arbitrary third-party trackers by reference). An optional `:small`/`:medium`/`:large` suffix — `image:<n>:large` — selects a display size; any other word after the colon (or no colon at all) is not this suffix, and a reference the fixed `small|medium|large` set does not recognise is treated as malformed and dropped like a foreign or nonexistent `<n>` (docs/specification-decision-log.md #23). Sized references are emitted as a raw `<img class="poll-image--<size>">` rather than through Markdown's own `![]()` syntax, since that syntax has no way to carry a class; `class` is therefore in the sanitiser's `img` allow-list below, but only ever populated from this fixed three-value table, never from operator text.
2. **YouTube.** The only video source either field can embed. Recognised in either of two forms, both extracted from the raw text by a fixed regular expression *before* Markdown or the sanitiser below ever see them, and replaced by an opaque placeholder token: a fenced block on its own lines — ` ```youtube ` on one line, an eleven-character video id on the next, ` ``` ` closing it — or a `youtube.com`/`youtu.be` URL standing alone on its own line, with the id read out of the `v=`, `/embed/` or `youtu.be/` part of it. A link that is part of a sentence, or written as `[texte](url)`, is left alone and stays an ordinary link; only a line that is nothing else is taken as a request to embed. After sanitisation, each placeholder is swapped for an `<iframe src="https://www.youtube-nocookie.com/embed/<id>">` that the renderer itself builds from the validated id — never from anything the operator wrote. This is the one point in the pipeline permitted to emit an iframe, the fixed `sandbox` attribute is `allow-scripts allow-same-origin allow-presentation` (playback only: no top-level navigation, no popups), and an id that fails the eleven-character check drops the whole block, or leaves the URL line as plain text, rather than guessing at what was meant.
3. **Everything else** goes through a Markdown parser and then an HTML sanitiser with a fixed allow-list — `p`, `br`, `strong`, `em`, `a`, `ul`, `ol`, `li`, `h2`–`h4`, `blockquote`, `img` and their few safe attributes (`href`, `src`, `alt`, `title`, and, on `img` only, `class`) — never `<iframe>`, `<script>`, an `on*` attribute, or a scheme but `http`/`https` on a link or an image. No raw HTML the operator wrote is ever passed through unsanitised, regardless of what Markdown extension might otherwise allow.

**Storage is content-addressed.** `PollImage.file`'s path is named after the SHA-256 digest of the bytes, under the owning poll's own id — never after the upload's own filename. This is what keeps the R-3.10 guarantee ("the public page cannot change under a viewer's eyes") extending to an image the way it already does to text: re-uploading edited bytes for what an operator thinks of as "the same picture" produces a new digest, a new path and a new row, so it can never land at the path a frozen `description`/`details` reference already points to. Re-uploading *identical* bytes for the same poll is a no-op, returning the existing row rather than a duplicate (`(poll_id, content_hash)` unique); `short_id` is likewise unique per poll (`(poll_id, short_id)`).

**Frozen alongside the rest of the configuration.** `description` and `details` sit on the `Poll`/`PollOption` rows respectively, so the *existing* `inv6_poll_config_frozen`/`inv6_option_*_frozen` triggers of §5 already cover them — no trigger change for either field. `PollImage` is a separate table, but unlike its option-scoped predecessor it carries its own `poll_id` directly, so its three triggers read the poll's state the same way every other INV-6 trigger does: no image may be inserted, updated or deleted once the owning poll is not `draft`. An image can therefore only ever be added to, or removed from, a poll's library while the poll — and its description and options — are all still freely editable; there is no path to add or swap an image once a poll has left `draft`, exactly as there is none to add or correct a translation (§3.8).

**Never touches the tally or the hash**, for the same reason labels do not (R-10.7): the canonical serialisation of §9 carries option ids and tracking codes only.

### 3.2 `RollEntry` — frozen snapshot (R-4.3)

`{poll_id, birth_name, usual_name, first_names, date_of_birth, date_uncertain, list_types}`. Copied from the current working roll at the `open` transition (§6.1). Immutable thereafter (INV-7).
...
- `source ∈ {online, paper}`.
- `status ∈ {live, superseded, deleted, pending_countersign}`. Exactly one row per logical ballot is `live`. `deleted` is a paper ballot withdrawn by an operator (R-8.5, R-9.4) — rows are never physically removed, save with the whole poll when a sandbox poll is deleted (§11 bis). `pending_countersign` is a paper entry awaiting a second operator where `paper_requires_countersign` is set.
...
### 3.5 bis `ReconciliationRecord` (R-8.6, §9)

`{id, poll_id, forms_retained_count, recorded_ballots_count, note, signed_by_id, signed_at}`. One per poll, `poll_id` unique. `forms_retained_count` is entered by the poll admin; `recorded_ballots_count` is computed from the live paper ballots at the instant of signing, never entered — the operator has no independent way to know it is right. `note` is free text, kept here rather than on an `AuditEvent`, whose `reason` may hold no prose (§10). Present only where `paper_requires_reconciliation` is set and the poll admin has signed it; its absence, with the flag set, is exactly what `closing_blockers` refuses closure over (§9).
...
- **Poll content** — `title`, `description`, and each option's `label` — is per-poll data and cannot live in a catalogue. Store it as `{language_code: text}` maps, or an equivalent translation table, keyed on the poll's enabled `languages`. An option's extended `details` (§3.1 bis, R-3.12) is stored the same way but is not one of these: it is enrichment, never a translation this rule requires.

Rules:

- A poll may not leave `draft` while any enabled language lacks a translation of the title, the description or any option label — `details` is deliberately absent from this list (§3.1 bis). The configuration screen shows the gaps.
...
`{id, name, data_protection_referent, data_protection_contact, public_base_url, legal_notice, logo, logo_content_type, logo_content_hash, logo_dark, logo_dark_content_type, logo_dark_content_hash, favicon, favicon_content_type, favicon_content_hash}`.

The one commune this instance serves (R-1.3, R-1.5): a singleton, `id` pinned to `1` by a check constraint, unlike every other table here. Created by the first-run wizard (§6.5.11) alongside the initial `commune_admin` account, and edited afterwards from screen 14 (§6.5.14) — the wizard and that screen are the only writers. `name`, `data_protection_referent` and `data_protection_contact` are R-13.1/13.2's data-controller identity, required at install. `public_base_url`, `legal_notice`, `logo`, `logo_dark` and `favicon` are all optional and independent of each other and of the required fields: blank means "use the deployment's own configuration" for the first, "render nothing" for the second, and "show nothing beyond the commune name" or "let the browser use its own placeholder" for the rest. `legal_notice` is free text rendered verbatim (line breaks kept, no Markdown) in the footer of every page, public site and espace mairie alike — the publisher notice French law expects of a public site, distinct from the R-13.1/13.2 identity above and not itself the subject of any numbered rule. `logo_dark` is a second, independently uploaded image swapped in for `logo` once the page's theme is dark (base.html, CSS-toggled by static/js/theme.js); it has no effect of its own where `logo` is blank, since there is then no image for it to replace. The `*_content_type`/`*_content_hash` pairs are how `logo`/`logo_dark`/`favicon` are stored content-addressed, the same shape `PollImage` uses for `content_hash` (§3.1 bis) — a changed image gets a new path, never an overwrite a cache could still be showing.

---

## 4. State machine

`draft → announced → open → closed → published`, irreversible (R-3.2). `announced` is a mandatory waypoint (R-3.10): a poll can only reach `open` by first being announced — there is no `draft → open` edge. A fifth, terminal state, `withdrawn`, branches off `announced`, `open`, `closed` or `published` — and only off those four (R-3.11) — and joins nothing: like `published`, no transition leaves it.

- `draft`: configuration mutable.
- `announced`: mandatory stop before opening. Configuration frozen (R-3.3) — the same trigger that freezes `open` fires here too, since both simply read `state != draft` (§5.1, INV-6); nothing distinguishes the two at that layer. Visible on the public site (§6.6): propositions and calendar shown, no registration and no vote offered, the page stating plainly that the poll is not yet open. No roll snapshot yet and no `opening_seed` — both remain `open`'s side effect (§6.1).
- `open`: configuration frozen (R-3.3) since the earlier stop at `announced`. Only `closes_at` may be extended, with a logged reason, displayed publicly (R-3.4); `opens_at` was already pulled back to the instant of opening if the poll was opened by hand ahead of it (R-3.4, below). Snapshot taken and `opening_seed` generated on entry.
...
- `withdrawn`: terminal, reached only from `announced`, `open`, `closed` or `published`, only by the poll admin's own action, and only with a mandatory reason (R-3.11). §6.6 governs what the public site does with it — nothing, on every page that would otherwise carry this poll — and §11 governs its one effect on data retention, for a poll withdrawn before ever reaching `closed`.

Extension of `closes_at` is permitted only while `state = open` **and** the clock has not yet reached the current `closes_at` — the same clock gate §6.6 uses for the public page's display — and only to a later timestamp; `paper_entry_deadline` moves with it, preserving the configured window length. `state = open` alone is not enough: it also covers the paper-keying stretch that follows `closes_at` (§6.4), during which "extending" would push `closes_at` back into the future and reopen online voting that had already, actually, stopped, rather than postpone one still running.

**Announcing is manual and mandatory, unlike every other transition.** There is no configured instant analogous to `opens_at` or `paper_entry_deadline` for `draft → announced` to fire at, so nothing schedules it: it exists solely as the poll admin's own action from screen 2 (R-2.1) — but every poll must pass through it, since `open_poll` no longer accepts `draft` as a source. `announcing_blockers` is lighter than `opening_blockers` in one respect only: it does not need a roll to snapshot, since none is needed for a preview and none is taken until the poll actually opens. It needs the same complete translation set opening does — announcing freezes the configuration exactly as opening does (R-3.3), so the public preview it produces must not show, or later reveal on opening, a translation gap that fell back silently while nobody but the poll admin could see it (§3.8) — plus the two-proposition minimum a public page must have something to show. It additionally needs `opens_at` still in the future: announcing a poll whose opening hour has already passed would freeze a public preview only to open immediately behind it, defeating the review this step exists to provide (R-3.10). A poll admin who missed the window recovers by pushing `opens_at` forward — an ordinary, freely permitted edit while still `draft` (R-3.3) — and then announcing.

**The other two transitions normally run unattended, and neither is trusted to be punctual.** `announced → open` is performed by the `open_poll` command at `opens_at`. `open → closed` is performed by `close_poll` at `paper_entry_deadline` — which equals `closes_at` unless a keying window is configured (§6.4). The job moves the state; the instants themselves are enforced independently on every ballot write path, which refuses a ballot falling outside the window for its source whatever `state` happens to say. A job that runs late, twice, or not at all therefore cannot admit a ballot outside that window — the reasoning of INV-2, applied at both ends. The write paths also require `state = open` (decision log #33): the clock refuses, the state admits. That only ever refuses more, so it cannot let a late job admit anything late; what it does mean is that a late `open_poll` delays the start of voting — which it did anyway, since registration, review and paper entry all need the snapshot it takes. A missed opening is therefore flagged on the dashboard and reported by `/sante` (§14). A `draft` poll nobody ever announced simply stays `draft`: the job never touches it, by design — an unreviewed configuration must never open unattended (R-3.10).
...
- `draft → announced` may be triggered manually at any time the guards above allow (translations complete, `opens_at` still in the future). `announced → open` may be triggered manually **at any time**, including ahead of `opens_at`, in which case **`opens_at` is set to the instant of opening** (R-3.4). The write paths check the clock against `opens_at` as well as `state` (above), so leaving `opens_at` at its configured value would have produced a poll that reads `open` in the back-office while the public site — which follows the clock — still says it has not started, and no ballot or registration would be accepted. Moving it makes the poll votable and listed from the moment the poll admin clicks. It is the only way `opens_at` ever changes after `draft`, and only earlier: the audit event of the transition carries the previous and the new value in `before`/`after`. The scheduled `open_poll` selects on `opens_at ≤ now` and never moves it.
- `open → closed` may be triggered manually **at any time**, including before `paper_entry_deadline` (R-3.4, decision log #34). Closed early, it is the mirror of an early opening: **`closes_at` and `paper_entry_deadline` are brought back to the instant of closure** — `closes_at` only if it is still in the future, so a closure during the paper-keying stretch leaves it where online voting actually stopped. A reason code is mandatory, and the move is logged as `poll_closed_early` with both instants before and after, and shown on the poll's public page beside any extension. Since every write requires `state = open` (§5.1), nothing is accepted after the transition, so `closure_hash` and the §9 counts cover exactly what was admitted. The guards are unchanged: an outstanding countersignature still needs the override reason, a missing reconciliation still refuses. The scheduled `close_poll` selects on `paper_entry_deadline ≤ now` and never closes early.

This is also where the admin override named just above gets its caller: `close_poll`'s `override_reason` parameter, mandatory for closing over a `pending_countersign` ballot, is supplied from this same manual path (§9, R-8.7 bis) — the scheduled command never supplies one.

Opening is the transition with side effects: it takes the roll snapshot (§6.1) and generates `opening_seed`, and both MUST occur in the same transaction as the state write, so that two concurrent runs cannot produce two snapshots or two seeds. Selection is state-based — `state = announced AND opens_at ≤ now` — so a host that was down opens the poll late rather than never, but only ever a poll a human already reviewed by announcing it.
...
**Opening can be refused, and a refusal must be loud.** The precondition that the poll actually be `announced` (not still `draft`), and — for the rare case a poll was announced before the roll was ever imported — the roll-to-snapshot check of §6.1, are checked by the job, at an hour when no operator is watching. `open_poll` refuses such a poll, leaves it `announced`, logs the blocking reason and exits non-zero; it never opens a partially configured poll. A silent non-opening being the worst outcome here, the dashboard (§6.5) names the blockers in advance rather than on the morning itself.

**Announcing can be refused too**, though nothing schedules it, so a refusal only ever happens the instant a poll admin clicks the button and the screen shows it immediately — there is no "at an hour nobody is watching" case to guard against here. `announcing_blockers` is the check above (translations, two-proposition minimum, `opens_at` still in the future); a refusal leaves the poll in `draft` and is logged the same way as the other two.
...
**Withdrawal is manual only, and unlike announcing it has no unattended counterpart at all** — nothing about *when* a poll should be pulled follows from a configured instant, so no scheduled command ever calls `withdraw_poll` (R-3.11). It accepts a poll in `announced`, `open`, `closed` or `published` and refuses every other source state, `draft` included — a poll nobody has seen yet is deleted, not withdrawn, and that path already exists on screen 2 without touching this state machine at all. Its reason is mandatory in the service function itself, not only in the form: `withdrawing_blockers` reports `not_withdrawable` for the wrong source state and `reason_required` for a missing one, exactly as the other guards report theirs. It is unrestricted in time, for the same reason opening and announcing are: nothing on any write path treats `state = withdrawn` as other than "refuse", so withdrawing sooner or later changes nothing about what a ballot, a registration or a publication already admitted. What withdrawal is not free to do is reach into the past: a ballot already cast stays cast, a closure hash already computed stays computed, and a result a third party already downloaded before the poll's page changed stays downloaded (R-3.11) — withdrawal governs what the platform shows from the moment it runs, not what already left it.
...
- **INV-6** Poll configuration fields other than `closes_at` are immutable once `state ≠ draft`, with one further carve-out: `opens_at` may change to an earlier instant in the very write that takes `announced → open` (R-3.4). Extends to `Poll.description` and `PollOption.details` (the same rows as the rest of the configuration and of `label` respectively, no separate trigger needed for either) and to `PollImage` (its own three triggers, reading the state through its own `poll_id` — §3.1 bis, R-3.12).
- **INV-7** `RollEntry` rows are never modified after the snapshot.
- **INV-8** Sandbox polls never appear in public listings, public results or aggregate statistics (R-3.7). Their poll page, registration form, ballot routes and, once published, results page are reachable only from a browser holding the poll's share link or a voter token of its own (§6.6), never from a public URL.
...
**Configuration immutability (R-3.3)** is enforced in `Poll.save()`: when `state != draft`, any change to a configuration field other than `closes_at` raises — save `opens_at` moving earlier on the `announced → open` write (R-3.4). Backed by a database trigger, since `save()` is bypassed by `update()`, `bulk_update()` and raw SQL.

**The voting window (INV-2)** is checked in one service function that every ballot write path calls, taking the poll and the current time, and rejecting a write outside the window for that ballot's source — before `opens_at`, at or after `closes_at` for an online ballot, at or after `paper_entry_deadline` for a paper one. It first requires `state = open` — refusing a `draft` or `announced` poll whose `open_poll` has not run, a `closed` or `published` one, and a `withdrawn` one (R-3.11) the instant it is withdrawn — then checks the clock. It never relies on `state` to refuse a write past a deadline, since the scheduled transition may not have run yet (§4): the closing bounds are the clock's alone (decision log #33). This is the invariant the typestate approach could not have delivered either, since elapsing time is not a method call; here as there, the clock is consulted at the point of use. The retention purge is the one writer permitted past `closes_at`; it goes through a separate, named service function of which it is the only caller. The INV-2 trigger carries the same `state = open` requirement on `INSERT` and `UPDATE`.
...
The administrative interface is purpose-built, not Django admin. It is used by council members and mairie staff, not by developers, and it therefore falls under RGAA like the rest of the site (R-14.1), must be in French, and must not expose destructive actions beside routine ones. Every definitive action on a poll (R-2.4) goes through a confirmation page: the action's form posts once to a page that restates what will happen and what cannot be undone — computed at that moment, such as the registrations still under review — and the action runs only on a second POST carrying the confirmation, so a forged or double-clicked single POST acts on nothing (decision log #35). Django admin is not included in the production URL configuration at all.
...
implemented, and tracked separately (docs/specification-decision-log.md #10).

1. **Tableau de bord** — state, opening and closing instants, registered / voted counts by channel (registered being the `active` registrations, the same figure §9 freezes at closure), pending review count, and the actions permitted in the current state. While the poll is in `draft` or `announced` it also names every condition that would make `open_poll` refuse — a missing translation, an absent roll snapshot — so a gap is visible before the opening hour rather than at it (§4). In `open` it likewise names what would block `close_poll` — *clôture bloquée : n bulletins en attente de contreseing*.
2. **Configuration du scrutin** — editable only in `draft`; read-only thereafter (including `announced`), with the closing-date extension (R-3.4) as a separate, reasoned action while `open` **and** online voting has not actually closed yet by the clock (§4) — the same "Le vote en ligne est clos." notice the dashboard and the public page already show replaces the extension form the instant that gate trips, rather than leaving it offering to "postpone" a vote that has already stopped. Also where R-2.1's manual *annonce, ouvre, clôt* is exercised (§4): *annoncer maintenant* while `draft`, at any time — mandatory, R-3.10, the only door to `open`; *ouvrir maintenant* while `announced`, at any time; *clôturer maintenant* while `open`, only once `paper_entry_deadline` has passed, with the countersignature-override reason (R-8.7 bis) offered alongside it. The `draft` screen additionally offers *partager cet aperçu* (R-3.10 bis, §6.6): generates, regenerates or revokes the unguessable link an outsider can hold without an account — independent of announcing, and of no effect once the poll leaves `draft`. From `announced`, `open`, `closed` or `published`, this screen also offers *retirer le scrutin* (R-3.11): a reasoned action like the extension and the closure override, but irreversible and with nothing else to configure afterwards — once posted, the screen itself becomes the `retiré` read-only view, naming the reason and the instant, since the poll admin who just withdrew it is not the public the rest of §6.6 excludes. A commune admin additionally sees *enregistrer comme modèle* here, in every state (§3.9) — the fields it reads are frozen from `draft` onward (INV-6), so there is nothing later that reading them sooner could get wrong.
3. **Import de la liste électorale** — commune-level, gated by the commune-admin flag rather than a poll's roles (R-2.1: importing the roll is the commune administrator's, not the poll administrator's), reached from the general menu and never from a poll's own. Upload, column mapping, validation report, preview, explicit confirmation (R-4.5), plus a name search over whatever `WorkingRollEntry` currently holds, for browsing the roll in force outside of an import. A poll's own menu carries a counterpart instead: while the poll is `draft` or `announced` it shows only which import is currently in force and when, since there is no frozen copy yet (R-4.3) — an `announced` poll has not taken its snapshot any more than a `draft` one has; once the poll reaches `open`, it becomes a paginated, searchable browse of that poll's own frozen copy, open to that poll's `poll_admin` and, per R-4.4, its `auditor` (docs/specification-decision-log.md #11). The `poll_admin` alone also sees, beside each entry, where that elector stands — no registration, unconfirmed, registered and not voted, voted online, voted on paper — read from `Registration.state` and `channel` (INV-5): the names-with-a-voted-flag list of R-7.5 and §7, so an elector who says they cannot vote can be told why. Never a timestamp beside a vote, which would line a registration up with a ballot (INV-1); never to the `auditor`, R-13.4 bis restricting participation by name to the poll admin.
4. **File d'attente des inscriptions** — registrations pending review, with roll search and near-match display; accept or reject with a mandatory reason (R-5.4). Below it, the electors matched but still in `pending_email` (R-5.5), oldest first, each with a *renvoyer le lien* action: it mints a fresh token, replacing `voter_hash`, and mails it (`services.resend_confirmation`, one `registration_link_resent` event naming the registration and no address, §10). The old link stops resolving; that is safe only because no ballot exists before confirmation, and the action refuses any other state. There is deliberately no operator-side *confirm* (decision log #28).
...
9. **Clôture et publication** — closure hash, tally derivation, tie-break computation where applicable, and the publication action. Where `paper_requires_reconciliation` is set, also where R-8.6's reconciliation record is entered: while the poll is still `open`, once `paper_entry_deadline` has passed (the same restriction screen 2 puts on the manual closure trigger, for the same reason — a count taken earlier could be made stale by a paper entry or correction the window still admits), the poll admin enters the count of paper forms retained by the commune; the count of paper ballots the system holds is computed, never taken on the operator's word. Signed and archived (§9); shown to the poll admin and the auditor, never published.
...
14. **Paramètres de la commune** — commune-level like screens 10, 12 and 13: the commune record screen 11 creates (name, data-protection referent and contact, R-13.1/2), editable afterwards rather than fixed at installation, plus the site's own address used to compose the links in outgoing mail (§6.2 step 7). Left blank, a poll's mail keeps using the deployment's own configuration (§14, §15), the same additive fallback screen 12 gives the mail relay — so an adopting commune is unaffected until an admin fills this screen in. Also here, both optional and independent of each other: a logo, shown in the page header in place of the plain commune name, and a favicon, the browser-tab icon. A third, dependent setting, `show_name_with_logo`, keeps the commune name printed as visible text beside a logo that does not carry it legibly on its own; it is ignored where no logo is set, since the name is then shown alone regardless. A fourth, also dependent, setting is a second logo for the page's dark theme (`logo_dark`), uploaded and removed the same way as the light one and shown in its place once the visitor's theme is dark; like `show_name_with_logo`, it is without effect where the light logo is not set. Logo, its dark variant, and favicon are content-sniffed and stored the same way as a proposition's image (R-3.12) — never trusted by declared content-type or filename extension, and never SVG, which this application has no means to sanitise before serving back.
...
"Open" here means votable, not `state == open`: the same clock that gates the write path (§5.1) gates the "Consultation ouverte." banner and the registration link. `state` can lag the clock in both directions — a poll admin may call `open_poll` ahead of `opens_at` by hand, and `close_poll` waits for `paper_entry_deadline` (§6.4), which the scheduled job reaching late in any case can stretch further still — and the page must not offer a vote the write path is already refusing, or withhold one it would already accept. Between `closes_at` and whichever of `paper_entry_deadline` or the job's next run comes later, `state` still reads `open` (paper keying and countersignature legitimately continue) but online voting has stopped; the page says so plainly instead of repeating "ouverte".

After publication the page carries the artefacts of §9.

**Early preview (R-3.10).** A `draft` poll is never public — no configuration turns it into one; `announced` is a distinct, mandatory waypoint (§4) reached only by a poll admin's own action, and it, not `draft`, joins `open`/`closed`/`published` as a state this section's query includes. An `announced` poll shows propositions and calendar, nothing else: no participation (`_live_participation` returns `None` off `open`), no extension list (none exists yet), and no registration link, since nothing behind one would accept a submission before the poll actually opens (§5.1). The page states plainly that the poll is not yet open and names the configured `opens_at`. Unlike a preview of a still-`draft` poll (below), this one shows a configuration that cannot change under a viewer's eyes: the transition into `announced` freezes it, the same trigger as `open` (INV-6). `is_sandbox` still wins (INV-8): a sandbox poll stays out of every public page regardless of state.

**Shared draft preview (R-3.10 bis).** A second, independent preview mechanism, this one genuinely for a still-`draft` poll — the "hypothetical" the paragraph above no longer needs to stay hypothetical about, because unlike that rejected design (`docs/specification-decision-log.md` #13) this one is never listed or publicly discoverable: it exists only at `/scrutin/<poll_id>/apercu/<token>/`, `token` an unguessable value the poll admin generates from screen 2 (`Poll.preview_token`, a lifecycle/access field deliberately outside `FROZEN_CONFIG_FIELDS` — INV-6 does not, and must not, freeze it). A request whose token matches renders exactly what the poll admin's own internal `poll_preview` screen shows — same context, same template chain (`publicsite/poll_detail.html`), different notice text — **without freezing anything**: the page can and does change between two visits, since the draft itself can, and it says so. A request naming a poll no longer `draft`, or one with no token set, or a mismatched token, 404s indistinguishably, save that a poll which has since left `draft` instead redirects to its real public page (`publicsite:poll_detail`) — a held link degrades gracefully into a bookmark rather than dying. Regenerating overwrites `preview_token` outright, immediately invalidating whatever anyone else was holding; revoking clears it. Neither is a state change, needs no reason, and both are logged (`Action.PREVIEW_LINK_GENERATED`/`PREVIEW_LINK_REVOKED`) without the token itself ever reaching the audit row (INV-3).

**Sandbox polls (R-3.7).** A sandbox poll is on no public page, yet must be triable by the people invited to. Its share link therefore keeps working in every state (`publicsite.poll_preview_shared`): while `draft` it renders the shared draft preview above; from `announced` on it renders `publicsite/poll_detail.html` itself (as `poll_sandbox.html`, the same template chain plus a notice that the poll is a rehearsal), registration link included whenever the poll is votable; once `withdrawn` it renders the withdrawn notice. What the link grants is recorded in the session as `{poll_id: token}` — the *presented token*, not a flag — and `sandbox.may_reach` compares it with the poll's current `preview_token` on every request, so regenerating or revoking the link ends the access of everyone who came in through the old one at the very next request. The registration routes require that grant. The ballot routes accept it or a *voter token*: `ballots.access` resolves a valid token on a sandbox poll without any grant (the confirmation mail is opened on whatever device the tester likes) and records `sandbox_voter` for the token-free pages that follow (§6.3); a wrong token on a sandbox poll with no grant is a bare 404 that names nothing. Neither grant carries a voter reference (INV-1). Every voter-facing page shows a one-line notice that the vote is a test.

A sandbox poll is registered against the roll like any other — the tester must be on it — because the point is to exercise the real path (R-5, §6.2). Its result appears on no public page (R-3.7): the operator reads it on screen 9 like any poll's, and once the poll is `published` the results page — HTML, CSV and JSON alike — answers a browser holding the link or a voter token of its own, with the rehearsal notice, and 404s for any other. Trying a poll end to end includes reading what it produced and running the verifier on it; the page leads back to the share link, never to the public poll page that does not exist for it.

The poll admin manages the link and deletes the poll from `poll_sandbox` (POLL_ADMIN acts, AUDITOR reads; a poll that is not a sandbox 404s): the link is generated, regenerated or revoked exactly as above (`elections.sharelink`, shared with the draft preview), and *supprimer* is a POST behind an explicit confirmation, offered in every state.

**Withdrawal (R-3.11).** `withdrawn` is deliberately absent from every query this section runs: the listing's, the detail page's and the results page's, CSV and JSON included, so a poll pulled after publication loses its artefacts exactly as completely as one pulled the day it was announced. It is absent from the listing outright — a withdrawn poll is not named there, not even as *retiré* — but its detail URL is not simply left to 404: an operator, a councillor or a member of the public may hold that link already (an email, a bookmark, a search index), and a plain 404 there reads as a broken site rather than a decision. The detail view answers such a request with a fixed notice — *ce scrutin a été retiré* — and nothing else: no title, no description, no propositions, no dates, no participation figures, no reference to a result that may once have been published there. The reason logged at R-3.11 is for the audit log and screen 2's own post-withdrawal view (§6.5), never for this page. A request for the results page, or its `?format=csv`/`?format=json`, against a withdrawn poll gets the ordinary 404 the same request gets against a poll that was never published at all — there being no notice to give an API client that makes sense, unlike the HTML page a person is reading.
...
Log at minimum: config changes; state transitions; `closes_at` extensions; roll imports; snapshots; registration review decisions; registration attempts against an already-registered roll entry; registrations refused for an ineligible list type; paper ballot create/correct/delete; countersignatures; reconciliation records (R-8.6); collision overrides with reason; role assignments; access to the log itself. Each entry: actor, timestamp, object, before, after, reason where required (R-12.2). Auditors read everything; nobody can modify or delete (R-12.3).
...
**A withdrawn poll (R-3.11) gets the same purge, anchored on `withdrawn_at` instead where the poll never reached `closed`.** Closure and withdrawal are alternative ends to the same thing — the poll being over — and R-13.3 anchors on whichever the poll actually reached: `closed_at` if the poll passed through `closed` (whether or not it went on to be withdrawn from `published`, or indeed from `closed` itself, in which case `closed_at` predates `withdrawn_at` and is the earlier, correct anchor), `withdrawn_at` otherwise — a poll pulled straight from `announced` or `open` has no `closed_at` at all, and without this second anchor its identity data would never become due. `due_polls` selects on whichever of the two is set and has aged past the retention period; a poll can supply at most one, since `closed_at` predates any `withdrawn_at` a later withdrawal from `closed` or `published` would set.

The purge is the sole exception to INV-2, which it necessarily breaches: it deletes `Registration` rows long after `closes_at`. Express the exception inside the trigger rather than around it — the INV-2 trigger forbids `INSERT` and `UPDATE` on `Registration` and `Ballot` outside the voting window, and permits `DELETE` of a registration only where the poll is `closed`, `published` or `withdrawn`. It must admit `closed` and not `published` alone: the anchor above is closure, and a poll that closes without ever being published is exactly the case the purge exists to reach; `withdrawn` joins them for the same reason a poll pulled before closure needs an anchor at all. Disabling the trigger for the duration of the job is not an acceptable substitute. The same applies to INV-7, whose trigger refuses every `UPDATE` on `RollEntry` unconditionally and permits `DELETE` only where the poll is `closed`, `published` or `withdrawn` — a snapshot is frozen, not immortal. INV-3 needs no such accommodation, since the purge never touches `AuditEvent` at all.

**Deleting a sandbox poll (§11 bis, R-3.7).** The one deletion of a whole poll, offered to its POLL_ADMIN in every state and to no other poll. `elections.sandbox.delete_poll` writes `Action.POLL_DELETED` (actor, `poll:<uuid>`, state and title in `before` — a title identifies no elector, §10), then removes, in the retention purge's order, the paper links, duplicate attempts, registrations and roll entries, and lets the rest — ballots, options, images, roles, reconciliation record — cascade from the poll; uploaded image files are removed from storage once the transaction commits. The audit log is never touched: `AuditEvent.poll` carries no database constraint and no cascade (audit migration 0011), so the events stay behind naming a poll that no longer exists, as they already do for a purged registration (§10). Because that constraint was what kept a *real* poll from being deleted, `inv3_poll_no_delete` now refuses every `DELETE` on `elections_poll` where `is_sandbox = 0`, and each delete trigger of INV-2 (registrations), INV-3 (ballots), INV-6 (options, poll images) and INV-7 (roll entries) gains one exemption, `is_sandbox = 1` (elections migration 0013). `is_sandbox` is frozen outside `draft` by INV-6 and a `draft` holds no ballot, registration or snapshot, so the flag cannot be flipped to reach real data. This is a departure from R-7.2 and R-12.3 that R-3.7 now states in terms; see decision log #26.
...
| T-15 | Sandbox poll, requested with no share link and no voter token: its detail, results, registration and ballot routes | Absent from public listings and statistics; every route 404s, with a bare 404 (no page naming the poll) on a wrong voter token (INV-8). Reachability with the link is T-87 |
...
| T-52 | Ballot posted before `opens_at`, with `state` forced to `open` directly in the database | Refused server-side by the voting-window check, which checks the clock as well as `state` and never relies on `state` alone (§4, INV-2) |
...
| T-67 | Screen 2's *ouvrir maintenant* posted on a fully configured, already-`announced` poll well before `opens_at`; and posted directly on a `draft` poll (skipping the announce step) | The first opens, snapshot and seed taken as the scheduled job would, and `opens_at` is now the instant of opening, the previous value logged; the poll is listed as open on the public site at once and accepts a ballot (§4, R-3.4). A raw `UPDATE` moving `opens_at` later, or on any other state, is rejected by the INV-6 trigger; the second is refused with the `not_announced` blocker `open_poll` would report, and stays `draft` |
| T-68 | Screen 2's *clôturer maintenant*: posted before `closes_at` without a reason, then with one; posted after `paper_entry_deadline` with a `pending_countersign` ballot outstanding and no reason given, then with a reason | Early without a reason the closure is refused and the poll stays `open`; with one it closes, `closes_at` and `paper_entry_deadline` become the instant of closure, a `poll_closed_early` event carries both instants and the reason, and the poll's public page shows it; any later ballot or registration is refused. After the deadline without a reason the override is refused; with one it closes and the override is logged and appears in the publication (§4, R-3.4, R-8.7 bis) |
...
| T-71 | `open_poll` called directly on an `announced` poll, and `UPDATE elections_poll SET state = 'open' WHERE state = 'closed'` issued in raw SQL | The first succeeds (roll snapshot, `opening_seed`, `POLL_STATE_CHANGED` recording `announced` as the prior state); the second is rejected by the `poll_state_irreversible` trigger, not merely by application code (R-3.2) |
| T-72 | `withdraw_poll` called with a reason on a poll in each of `announced`, `open`, `closed` and `published` | All four succeed, land in `withdrawn`, logged with the reason; a further transition attempted on any of them (including another withdrawal) is refused (R-3.11, R-3.2) |
| T-73 | `withdraw_poll` called on a `draft` poll, and called with no reason on an `open` one | Both refused, stay where they were, logged as `job_refused`; the first names `not_withdrawable`, the second `reason_required` (R-3.11) |
| T-74 | `UPDATE elections_poll SET state = 'withdrawn' WHERE state = 'draft'` issued in raw SQL, and `UPDATE ... SET state = 'open' WHERE state = 'withdrawn'` | Both rejected by the `poll_state_irreversible` trigger (R-3.2, R-3.11) |
| T-75 | Public listing, detail page, results page, CSV and JSON, each requested for a poll withdrawn from `announced`, from `open` and from `published` | Listing never names it, in any of the three cases; detail page renders only *ce scrutin a été retiré*, with no title, description, proposition, date or reason; results page, CSV and JSON all 404, including the `published` case where they had briefly existed (R-3.11, §6.6) |
| T-76 | Retention job run two months after a poll is withdrawn from `announced` (no `closed_at` ever set), and, separately, two months after a poll is withdrawn from `published` (both `closed_at` and a later `withdrawn_at` set) | Both purge on schedule — the first anchored on `withdrawn_at` for lack of any other anchor, the second on the earlier `closed_at`; identity data gone in both, ballots and the audit log untouched (R-13.3, §11) |
| T-77 | `DELETE FROM elections_rollentry` issued in raw SQL against a poll still `open`, then again once the same poll is withdrawn | Refused in the first case (INV-7); succeeds in the second, exactly as it already does for `closed` or `published` — the trigger admits the state, not the elapsed time, which is the job's own selection to enforce (§11) |
| T-78 | A poll withdrawn from `open`, well inside its own voting window (`opens_at < now < closes_at`): an online ballot posted, a registration submitted, and, in raw SQL, `INSERT` and `UPDATE` attempted directly against `ballots_ballot` and `registrations_registration` | All refused — the application check first, then the INV-2 trigger for the raw SQL — even though the clock alone would still admit every one of them; withdrawal stops new writes immediately, since every write requires `state = open` (R-3.11, §5.1, decision log #33) |
| T-79 | An option's `details` rendered with: a YouTube fence carrying a valid id; one carrying `"; DROP TABLE elections_poll; --"` in place of an id; a bare `https://youtu.be/<id>` link alone on its own line; that same link written as `[texte](url)` and again mid-sentence; raw `<script>` and `<iframe>` tags typed directly into the Markdown; an `image:<n>` reference to an image that belongs to a *different poll*; and one to a `short_id` that does not exist at all in this poll | The valid fence and the bare link each become an `<iframe>` in the output, both pointed at `youtube-nocookie.com`; the invalid id drops the whole block with no iframe and no error; the `[texte](url)` form and the mid-sentence mention stay ordinary links, not embeds; the typed `<script>`/`<iframe>` are stripped by the sanitiser and never reach the page; both foreign and non-existent image references are dropped silently, leaving the rest of the text intact (§3.1 bis, R-3.12) |
| T-80 | `INSERT`/`UPDATE`/`DELETE` on `elections_pollimage` attempted in raw SQL against a poll that is `open`; the same three against a poll still `draft`; and an image re-uploaded with byte-identical content to one already in the same poll's library | The three against the `open` poll are all rejected by the `inv6_pollimage_*_frozen` triggers; the same three against the `draft` poll succeed; the identical re-upload returns the existing row rather than creating a duplicate, and a second, distinct image re-uploaded to the same poll succeeds as a separate row with the next `short_id` (§3.1 bis, R-3.12) |
| T-86 | An `image:<n>` reference rendered with a `:small`, `:medium` and `:large` suffix in turn; the same reference with no suffix; an override alt text containing a `"`; and a suffix outside the fixed set (`image:<n>:huge`) | Each of the three named suffixes emits an `<img class="poll-image--<size>">`; no suffix emits the unchanged, class-free `<img>` of before this feature; the quoted alt text is HTML-escaped rather than breaking out of the attribute; the unrecognised suffix is not treated as a size at all and the whole reference is dropped, exactly like a foreign or nonexistent `short_id` (§3.1 bis, R-3.12) |
| T-81 | A poll with `paper_requires_reconciliation` set: closure attempted before the reconciliation record is entered, including with a mandatory reason supplied as if for the countersignature override; the record entered before `paper_entry_deadline`; the record entered after it, with paper ballots of every status seeded beforehand | The first two are refused, no override accepted, poll stays `open`; the third is refused with the same blocker; the fourth succeeds, `recorded_ballots_count` counting only the `live` paper ballots, and closure then proceeds (R-8.6, §9) |
| T-82 | `open_poll` command run, and `open_poll` called by hand from screen 2, both against a fully configured `draft` poll whose `opens_at` has passed, never announced | Both refuse with the `not_announced` blocker, the poll stays `draft`; the scheduled command exits non-zero and logs the blocker (R-3.2, R-3.10, §4) |
| T-83 | `announce_poll` called on a `draft` poll whose `opens_at` has already passed; then, on the same poll, `opens_at` moved forward and `announce_poll` called again | The first refuses with `opens_at_not_in_future`, poll stays `draft`; moving `opens_at` forward succeeds (R-3.3, ordinary draft edit) and the second `announce_poll` call succeeds (R-3.10) |
| T-84 | A share link generated for a `draft` poll and fetched with no session at all; the poll's title changed and the same link fetched again; the link fetched with the token altered by one character; a poll with `preview_token` blank fetched with an empty token segment | The first two both 200 and show the current title each time — the second reflecting the change, unlike the frozen `announced` preview of T-69; the third and fourth both 404, indistinguishable from each other (R-3.10 bis) |
| T-85 | A share link regenerated, then fetched with the old token; a share link revoked, then fetched; a share link fetched once the poll has been announced and once opened | The first two both 404; the third and fourth both redirect to `publicsite:poll_detail`, which then renders whatever that state's own rules give (R-3.10 bis) |
| T-87 | A sandbox poll's share link fetched once `open`; the registration form fetched by that browser and by one that never used the link; the link regenerated, then the same browser's registration fetched again | The link serves the poll page with the registration link and the rehearsal notice; the registration form is 200 for the first browser and 404 for the second; after regeneration the first browser's grant is void and it 404s too (R-3.7, INV-8) |
| T-88 | A tester registers on a sandbox poll through the link, opens the mailed voter link in a browser with no cookie, casts, and reads the receipt; the same voter route hit with a wrong token | The ballot is stored and the receipt page renders with the rehearsal notice; the wrong token is a bare 404 (R-3.7) |
| T-89 | A sandbox poll holding a registration, a cast and a superseded ballot, deleted in each of `draft`, `announced`, `open` and `closed`; the same raw `DELETE` on ballots, registrations, roll entries, options and the poll row itself issued against a poll that is not a sandbox; `DELETE` and `UPDATE` on `audit_event` after the deletion | Every sandbox deletion succeeds and leaves no row of the poll; every raw `DELETE` against the real poll is rejected by its trigger; both statements on the audit table are rejected (INV-2, INV-3, INV-6, INV-7, R-3.7) |
| T-90 | A sandbox poll deleted | The audit events it had before are all still present, plus one `poll_deleted` naming the actor, the poll and its former state and title; the service refuses a poll that is not a sandbox (INV-3, R-3.7) |
| T-91 | A paper ballot keyed then deleted on a roll entry, and a registration in `pending_review` that a poll admin approves onto that entry; the same approval with the paper ballot still live | The applicant's row is bound and goes to `pending_email`, the shell is `rejected` and unbound, the event names it under `replaces`, one non-rejected registration remains; with a live paper ballot the approval is refused (R-9.4, R-5.9) |
| T-92 | A `pending_email` registration has its link resent; the same on an `active`, `pending_review` and `rejected` one, and after `closes_at` | The new token resolves to the registration and the old one to nothing, state unchanged, one event carrying no address; every other state and a closed window are refused with no event (R-5.5, R-7.6) |
| T-93 | Each definitive action of R-2.4 posted once without confirming, then confirmed | The first POST changes nothing and renders a page naming the action and its consequences, with the operator's input carried over; only the confirming POST acts; a confirming POST from an operator without the role is refused as before (R-2.4, §6.5) |
| T-94 | A sandbox poll published; its results page, CSV and JSON fetched by a browser holding the share link, by one holding none, and by the first after the link is regenerated | 200 with the rehearsal notice and a way back to the link for the first; 404 for the second and for the first once its grant is void; the poll is on no public listing throughout (R-3.7, INV-8) |
...
**Health endpoint.** `GET /sante` returns 200 with the application version, whether migrations are pending, and `transitions_overdue`: whether any poll is past the instant a scheduled transition should have moved it — `announced` past `opens_at`, or `open` past `paper_entry_deadline` — by more than a grace period, so monitoring catches a scheduler that stopped (decision log #33). No authentication, no personal data, no counts. Used by the Ansible smoke play (§15) and by monitoring.
...
**Independent verifier in Rust.** A second implementation of §8 and §9 as a small standalone binary, written from this specification, sharing no code with the application — a property the language boundary enforces structurally. Publish prebuilt binaries for the common platforms so a sceptic needs no toolchain. It reads only what the site publishes, never the database: the JSON publication document by preference, whose layout `docs/publication-format.md` fixes and versions, or the CSV. From the ballots it recomputes the closure hash, the pairwise matrix, the winner under the poll's method (Schulze, plurality or approval, R-10.3) and the tie-break, and from the document it checks each of those published values, and the counts, against its recomputation. The method is the one thing it takes on trust: the document states it, and the verifier restates it for comparison with what the poll announced. The CSV carries ballots only, so there the caller supplies the method, the option list and the values to compare (decision log #37). CI asserts agreement with the Python tally on fixtures including the cyclic case at T-9 and on randomly generated ballot sets. When the two disagree, the resolution is to return to this specification and determine which is wrong, never to adjust the verifier until it matches.
...
8. Backups: nightly `VACUUM INTO` to `/var/backups/`, alongside a `tar` of `DJANGO_MEDIA_ROOT` (poll images, R-3.12; the commune logo and favicon, §6.5.14) stamped with the same timestamp, retention window, and off-host replication of both. The database contains `poll.token_salt`, so backup destination permissions are part of the ballot-secrecy boundary and the playbook must not loosen them.
