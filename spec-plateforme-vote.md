...
Publish (R-11.2): anonymised ballot list as CSV and JSON (tracking code + ranking); pairwise matrix; derivation; the participation counts frozen at closure — registered electors, ballots by channel, non-voters, and the paper entries a countersignature override left uncounted (R-8.7 bis), as a figure of their own so that the paper count matches the ballot list; `closure_hash`; `opening_seed`; tie-break computation if any.
