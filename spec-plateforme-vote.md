...
2. **YouTube.** The only video source `details` can embed. Recognised as a fenced block on its own lines — ` ```youtube ` on one line, an eleven-character video id on the next, ` ``` ` closing it — extracted from the raw text by a fixed regular expression *before* Markdown or the sanitiser below ever see it, and replaced by an opaque placeholder token. After sanitisation, each placeholder is swapped for an `<iframe src="https://www.youtube-nocookie.com/embed/<id>">` that the renderer itself builds from the validated id — never from anything the operator wrote. This is the one point in the pipeline permitted to emit an iframe, the fixed `sandbox` attribute is `allow-scripts allow-same-origin allow-presentation` (playback only: no top-level navigation, no popups), and an id that fails the eleven-character check drops the whole block rather than guessing at what was meant.
...
| T-79 | An option's `details` rendered with: a YouTube fence carrying a valid id; one carrying `"; DROP TABLE elections_poll; --"` in place of an id; raw `<script>` and `<iframe>` tags typed directly into the Markdown; an `image:<uuid>` reference to an image that belongs to a *different* option; and one to an id that does not exist at all | The valid fence becomes the one and only `<iframe>` in the output, pointed at `youtube-nocookie.com`; the invalid id drops the whole block with no iframe and no error; the typed `<script>`/`<iframe>` are stripped by the sanitiser and never reach the page; both foreign and non-existent image references are dropped silently, leaving the rest of the text intact (§3.1 bis, R-3.12) |
