...
- **INV-8** Sandbox polls never appear in public listings, published results or aggregate statistics (R-3.7). Their poll page, registration form and ballot routes are reachable only from a browser holding the poll's share link or a voter token of its own (§6.6), never from a public URL.
...
A sandbox poll is registered against the roll like any other — the tester must be on it — because the point is to exercise the real path (R-5, §6.2). Its results are not published on the public site (R-3.7); the operator reads them on screen 9 like any poll's.
