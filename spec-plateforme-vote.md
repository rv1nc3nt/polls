...
A second, independent purge covers `WorkingRollEntry` (R-13.3 bis): two months after **import**, not closure — there is no poll to anchor it on before one opens, and none of R-13.3's purge triggers apply to a roll nothing has frozen yet. "In use" means a poll currently in `draft`, the only state that still reads `WorkingRollEntry` at `draft → open`; an `open`, `closed` or `published` poll already holds its own `RollEntry` copy and never looks at the working roll again, so its existence does not postpone this purge. The job deletes `WorkingRollEntry` rows only — `RollImport` (filename, hash, row count, operator, date) is provenance, not identity data, and stays, exactly as the per-poll purge leaves `AuditEvent` alone. No trigger backs this one: unlike `RollEntry` (INV-7), `WorkingRollEntry` was never protected against deletion — `apply_import` already replaces it wholesale at will — so the two-month rule is enforced by the job's selection logic alone, same as every retention rule's *when*, as distinct from the triggers that enforce the *window* R-13.3's purge must cut through.
...
| T-66 | Working-roll purge job run two months after import, once with a `draft` poll pending and once with none | Purges only in the second case; `RollImport` provenance intact either way (R-13.3 bis) |
