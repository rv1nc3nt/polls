...
- `open`: configuration frozen (R-3.3) since the earlier stop at `announced`. Only `closes_at` may be extended, with a logged reason, displayed publicly (R-3.4). Snapshot taken and `opening_seed` generated on entry.
...
- `draft → announced` may be triggered manually at any time the guards above allow (translations complete, `opens_at` still in the future). `announced → open` may be triggered manually **at any time**, including ahead of `opens_at`. This is safe rather than merely convenient: nothing on the ballot or registration write paths depends on `state`, only on the clock against `opens_at` itself (above), so opening early freezes the roll snapshot and draws `opening_seed` sooner but admits no ballot and no registration before the configured instant.
...
- **INV-6** Poll configuration fields other than `closes_at` are immutable once `state ≠ draft`. Extends to `Poll.description` and `PollOption.details` (the same rows as the rest of the configuration and of `label` respectively, no separate trigger needed for either) and to `PollImage` (its own three triggers, reading the state through its own `poll_id` — §3.1 bis, R-3.12).
...
| T-67 | Screen 2's *ouvrir maintenant* posted on a fully configured, already-`announced` poll well before `opens_at`; and posted directly on a `draft` poll (skipping the announce step) | The first opens, snapshot and seed taken, identical in effect to the scheduled job (§4); the second is refused with the `not_announced` blocker `open_poll` would report, and stays `draft` |
