...
**R-3.2** Les changements d'état sont strictement ordonnés : `projet → [annoncé] → ouvert → clos → publié`. L'état `annoncé` est une étape facultative (R-3.10) : un scrutin peut aussi passer directement de `projet` à `ouvert`. Depuis les états `annoncé`, `ouvert`, `clos` ou `publié`, et depuis eux seuls, un scrutin peut en outre être retiré (R-3.11), vers un état `retiré` qui met fin à son cycle de vie. Aucune transition n'est réversible.
