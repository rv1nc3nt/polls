...
destructrice à côté d'une action de routine**.

> **Ce que l'espace mairie ne permet pas.** Associer un électeur ayant voté **en
> ligne** au bulletin qu'il a déposé. Aucun rôle ne le permet.
...
> identifiés par leur code de suivi. La seule exception voulue est
...
Chaque opérateur a un **compte nominatif**. Les comptes partagés sont
...
1. **Première installation** (`/mairie/installation/`) — écran 11. Disponible
   ...
   - la **fiche commune** : nom affiché sur les pages publiques, **référent
     ...
     adressent accès, rectification, effacement) ;
   ...
   **paramètres de la commune** (écran 14).
...
3. **Import de la liste électorale** (écran 3).
4. **Créer le premier scrutin** (ci-dessous) puis **le configurer**.
...
actif** en ligne et chaque **rôle** en colonne : une case cochée
...
au journal, une attribution ou un retrait à la fois. Un compte
...
Toute attribution de rôle est tracée au journal.
...
l'écran 2 — la configuration initiale a la même forme qu'une
modification — en y ajoutant un seul champ que l'écran 2 exclut : le
**bac à sable**, fixé une fois pour toutes à la création.
...
créer : la validation renvoie directement vers « Rôles par scrutin »
...
*enregistrer comme modèle*, disponible sur l'écran 2 **quel que soit
l'état du scrutin**, puisque les champs qu'il copie (méthode, contraintes du
bulletin) sont déjà figés dès la sortie du brouillon. Supprimer un
...
scrutin continue d'utiliser la configuration du déploiement : remplir cet
écran est donc sans risque, une
...
**Aucune transition n'est réversible**. `Poll.state` n'est écrit que
...
- **brouillon → annoncé** (**obligatoire**) : rend le scrutin visible
  ...
  que l'ouverture l'aurait fait (même déclencheur), pour qu'elle ne
  ...
  tirée qu'à l'ouverture. **Également refusée si la date d'ouverture est
  ...
  `announced`, voir le guide de l'administrateur système), donc annoncer un
  ...
  d'abord la date d'ouverture (modification libre en brouillon), puis
  annonce.
- **annoncé → ouvert** : fige une **copie immuable de la liste électorale**
  et tire la **graine d'ouverture** (pour un éventuel départage). C'est
...
- **annoncé, ouvert, clos ou publié → retiré** (à tout moment, motif
  **obligatoire**) : plus rien du scrutin ne reste sur le site public — ni
  ...
  données, un scrutin retiré avant d'avoir été clos obtient, faute de
...
Un scrutin comprend : titre ; description ; **liste ordonnée d'au moins
...
quitte le brouillon** — à l'annonce, la seule porte de sortie du
...
libellés et de leurs traductions. **Ne changez pas un identifiant après
coup** : les bulletins et le résultat publié le portent.

Le nombre de propositions n'est pas limité (**au moins deux**). Chaque
...
bulletin de vote tire son propre ordre pour chaque électeur,
...
ne portent que sur les identifiants.
...
**tant que le scrutin est en brouillon** — remplacer une image
...
qu'une traduction manque** ; une traduction absente retombe sur la
...
**Figure 13 — Configuration modifiable (scrutin en brouillon), avec *Annoncer maintenant* en bas de formulaire — la seule action de transition offerte tant que le scrutin est en brouillon.**
...
**Champs importés, et eux seuls** : nom de naissance, nom d'usage,
...
numéro d'ordre n'est ni unique ni stable.) L'adresse n'est importée que
...
papier : elle répond à « qui figure sur la liste en ce moment », pas à
« quelle entrée correspond à la personne présente ».

### La copie figée d'un scrutin reste consultable
...
### Rétention de la liste de travail non consommée
...
planifié, même verrou que la purge par scrutin ; seules les entrées de
...
- **Signalé et importé** : une date de naissance qui ne s'analyse pas (ligne
  marquée « date incertaine », conservée telle quelle) ; une ligne
...
Une inscription est **mise en examen** quand : aucune entrée ne
...
### Exigences formelles, par scrutin
...
Dans tous les cas, le **noyau minimal de traçabilité**
s'applique.
...
2. **Vérifier le canal de vote** de l'électeur :
   ...
   - **en ligne déjà enregistré** → **saisie refusée**. Un bulletin voté en
     ligne est anonyme et ne peut pas être localisé depuis l'inscription : il
     ne peut être ni remplacé, ni supprimé. Si le scrutin autorise
...
et de suppression éventuelle à la demande.
...
d'audit indiquant l'opérateur, l'horodatage et l'état **avant/après**.
...
### La clôture refuse d'abandonner des bulletins en silence
...
Présent **uniquement** si `paper_requires_reconciliation` est activé. Une
...
Contrairement au contreseing, **il n'existe pas de passage outre** :
...
contreseing, il doit être « signé et archivé », pas mis à la disposition du
public.
...
Le **résultat lui-même est mis en avant**, pas seulement disponible : sur la
page de résultats, le gagnant (ou l'égalité, ou l'absence de bulletin retenu)
ouvre la page dans un encart séparé, avant la méthode, la
...
Le journal enregistre au minimum : modifications de configuration ;
...
bulletins papier ; passages outre à l'avertissement de
...
- **La commune est responsable de traitement**. Le traitement figure au
  registre des activités de traitement.
- Une **notice d'information** est présentée à l'inscription (finalité, base
  légale, durées de conservation, destinataires, droits, identité du
  référent). Le référent est celui saisi à la première installation,
  modifiable ensuite depuis les paramètres de la commune (écran 14).
- **Durées de conservation** : les données d'identité (inscriptions,
  copie figée de la liste, association bulletin papier ↔ électeur) sont
  supprimées **deux mois après la clôture**, ou après le **retrait**
  ...
  **clôture**, ou à défaut le **retrait**, jamais la publication (un scrutin
  ...
  obéit à une règle
  voisine mais distincte : elle est supprimée **deux mois après
...
- L'accès aux **données d'identité** est restreint à l'**administrateur du
  scrutin** (risque résiduel dont l'acceptation relève de la commune).
...
les trois champs que l'écran 11 fixe à l'installation : le **nom de la
commune** (affiché sur les pages publiques et dans la notice d'information),
le **référent données personnelles** et son **contact**.
...
comme les images d'une proposition — un champ de fichier laissé vide n'y
...
configuration d'un scrutin ou le relais de messagerie de l'écran 12.
