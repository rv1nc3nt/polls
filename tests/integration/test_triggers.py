...
def test_t80_poll_image_frozen_outside_draft(open_window_poll: Poll) -> None:
    """INV-6 extended to ``PollImage`` (R-3.12, §3.1 bis).

    Unlike the per-option table it replaced, this one carries its own
    ``poll_id`` directly, so "frozen outside draft" reads it the same way
    every other INV-6 trigger does.
    """
    image = PollImage.objects.create(
        poll=open_window_poll, short_id=1, content_type="image/png", content_hash="a" * 64
    )
    raw("UPDATE elections_poll SET state = 'announced' WHERE id = %s", [pk(open_window_poll)])
    raw("UPDATE elections_poll SET state = 'open' WHERE id = %s", [pk(open_window_poll)])
    ...
    with pytest.raises(Exception, match="INV-6"), transaction.atomic():
        raw(
            "INSERT INTO elections_pollimage "
            "(id, poll_id, short_id, file, content_type, content_hash, alt_text, uploaded_at) "
            "VALUES (%s, %s, 2, '', 'image/png', %s, '', %s)",
            [uuid.uuid4().hex, pk(open_window_poll), "b" * 64, sql_time(timezone.now())],
        )
    with pytest.raises(Exception, match="INV-6"), transaction.atomic():
        raw("UPDATE elections_pollimage SET alt_text = 'x' WHERE id = %s", [pk(image)])
    with pytest.raises(Exception, match="INV-6"), transaction.atomic():
