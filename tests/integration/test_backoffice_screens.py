...
def test_the_dashboard_names_what_would_stop_the_announcing(
    client: Client, open_window_poll: Poll, admin_user: User
) -> None:
    """§6.5.1: while in ``draft`` it names every condition that would make
    ``announce_poll`` refuse — the poll's next transition, now that
    ``announced`` is mandatory (R-3.10) — so a gap is visible before the
    opening hour (§4)."""
    ...
    assert "ne peut pas être annoncé" in body
