...
Usage:  python3 generate_fixture_roll.py [--rows 60] [--seed 1] [--out FILE]
"""
import argparse, random, unicodedata

COLUMNS = [
    "code du l'ugle", "libellé de l'ugle", "libellé du type de liste",
    "nom de naissance", "nom d'usage", "prénoms", "sexe", "date de naissance",
    "code commune de naissance", "libellé commune de naissance",
    "code département de naissance", "pays de naissance", "libellé nationalité",
    "numéro de voie", "libellé de voie", "complément 1", "complément 2",
    "lieu-dit", "code postal", "commune", "pays",
...
COMMUNE_SPELLINGS = ["SAINT EXEMPLE EN FIXTURE", "Saint-Exemple-en-Fixture",
                     "ST EXEMPLE EN FIXTURE", "SAINT-EXEMPLE-EN-FIXTURE"]

SURNAMES = ["ABADIE", "BALTHAZAR", "CARBONNEL",
            "DELAVIGNE", "ESCOFFIER", "FALGUIERE",
            "GRANDMOUGIN", "HAUTEFEUILLE", "IRIGARAY",
            "JOUBERT-VALLE", "KERGUELEN", "LAVOISIER",
            "MONTGOLFIER", "NOIRTIER", "ORSATTI",
            "PELLETANT", "QUENTIN-BRUYERE", "RAVANEL",
            "SOUBEYRAN", "TREMBLAY", "URBANI",
            "VAUCANSON", "WATTEBLED", "XAINTRAILLES",
            "YVERNAULT", "ZAMPIERI", "BRESSANDE",
            "CHANTEMERLE", "DAUVERGNE", "ENGILBERT",
            "FOUGEROLLE", "GRIVOLLET", "HUBERDEAU",
            "JASSERAND", "LOMBARDOT", "MAUFFREY",
            "NIVOLLET", "PERRACHON", "ROUSSILLE",
            "SAVOURNIN", "TOURNADRE", "VUILLERMOZ",
            "BOISSERAND", "CHAMPOLLION"]
ODD_SURNAMES = ["DE LA ROCHEFOUCAULD", "LE BRETHON", "DU PLESSIS-MORNAY",
                "D'AUBIGNE", "SCHÖNBERGER", "TRAN VAN",
                "O'SULLIVAN", "SAINT-AMOUR", "VAN DER MEULEN",
                "ÉLÉONORE-BLIN"]
M_FORENAMES = ["Aurélien", "Baptiste-Amaury", "Corentin",
               "Ewen", "Hilaire", "Isidore",
               "Joachim", "Kilian", "Ludovic",
               "Marceau", "Nathanaël", "Octave",
               "Prosper", "Raphaël", "Séverin",
               "Tanguy", "Ulysse", "Victorin",
               "Wilfried", "Zéphirin", "Barnabé",
               "Clovis", "Dorian", "Eloi",
               "Gontran", "Fulbert", "Amaury",
               "Ansgar"]
F_FORENAMES = ["Apolline", "Capucine", "Eugénie",
               "Faustine", "Gwenaëlle", "Hortense",
               "Iseult", "Joséphine", "Léonie",
               "Maëlys", "Noémie", "Ombline",
               "Pénélope", "Quitterie", "Rosalie",
               "Sidonie", "Tiphaine", "Ursule",
               "Violaine", "Xavière", "Yolande",
               "Zélie", "Bérangère", "Clémence",
               "Domitille", "Églantine", "Armelle",
               "Solange"]
STREETS = ["Rue du Cadran-Solaire", "Route des Charbonniers",
           "Chemin de la Sauge", "Place du Vieux-Pressoir",
           "Rue des Quatre-Vents", "Impasse du Colombier",
           "Route de la Ganterie", "Chemin des Corbeaux",
           "Allée des Sorbiers", "Rue du Pont-Levant"]
HAMLETS = ["Le Serre-Bariol", "Les Granges-Neuves",
           "La Croix-Frimont", "Le Clos-Pigeon"]
BIRTH_FR = [("75056", "PARIS", "75"), ("13055", "MARSEILLE", "13"),
            ("69123", "LYON", "69"), ("31555", "TOULOUSE", "31"),
            ("06088", "NICE", "06"), ("44109", "NANTES", "44"),
            ("34172", "MONTPELLIER", "34"), ("67482", "STRASBOURG", "67"),
            ("33063", "BORDEAUX", "33"), ("59350", "LILLE", "59")]
# inconsistent country spellings and a historic entity, as real exports contain
BIRTH_ABROAD = [("", "BRUXELLES", "99", "BELGIQUE"),
                ("", "CASABLANCA", "99", "MAROC"),
...
class Builder:
    ...
    def person(self, sex=None, surname=None, forenames=None, dob=None, **kw):
        ...
        dob = dob or "%02d/%02d/%d" % (self.rng.randint(1, 28),
                                       self.rng.randint(1, 12),
                                       self.rng.randint(1935, 2008))
        if "nom d'usage" not in kw and sex == "F" and self.rng.random() < 0.35:
            other = self.rng.choice(SURNAMES)
            if other != surname:
                kw["nom d'usage"] = other
        return self.row(**{"nom de naissance": surname, "prénoms": forenames,
                           "sexe": sex, "date de naissance": dob, **kw})
...
def build(rows_wanted, seed):
    ...
    # --- deliberate edge cases, documented in FIXTURES.md ---

    # 1. married name: nom d'usage differs from birth name
    b.person(sex="F", surname="DELAVIGNE", forenames="Apolline Capucine",
             dob="14/03/1962", **{"nom d'usage": "RAVANEL"})
    # 2. nom d'usage identical to birth name (happens in real exports)
    b.person(sex="F", surname="SOUBEYRAN", forenames="Léonie", dob="02/11/1971",
    ...
    b.person(sex="M", surname="VUILLERMOZ", forenames="Barnabé Clovis Isidore Ulysse",
             dob="05/02/1954")
    ...
    b.person(sex="M", surname="ORSATTI", forenames="Zéphirin", dob="00/00/1953",
             **{"pays de naissance": "ESPAGNE", "libellé commune de naissance": "MADRID",
                "code commune de naissance": "", "code département de naissance": "99"})
    b.person(sex="F", surname="TRAN VAN", forenames="Quitterie", dob="1961",
             **{"pays de naissance": "VIET NAM", "libellé commune de naissance": "",
                "code commune de naissance": "", "code département de naissance": "99"})
    ...
    for surname, forenames, sex, dob, nat in [
        ...
        common = {"nom de naissance": surname, "prénoms": forenames, "sexe": sex,
                  "date de naissance": dob, "libellé nationalité": nat,
                  "code commune de naissance": "", "code département de naissance": "99",
    ...
    b.person(sex="F", surname="ZAMPIERI", forenames="Églantine", dob="08/01/1983",
             **{"libellé du type de liste": COMP_EUR, "libellé nationalité": "Italienne",
                "libellé commune de naissance": "TORINO", "code commune de naissance": "",
                "code département de naissance": "99", "pays de naissance": "ITALIE"})
    # 10. incomplete rows: no polling station; no street number; only a hamlet
    b.person(**{"code du bureau de vote": "", "libellé du bureau de vote": "",
                "code circonscription législative du bureau de vote": "",
                "libellé canton du bureau de vote": "",
                "numéro d'ordre dans le bureau de vote": ""})
    b.person(**{"numéro de voie": ""})
    b.person(**{"numéro de voie": "", "libellé de voie": "",
                "lieu-dit": rng.choice(HAMLETS)})
    ...
    seen = {(norm(r["nom de naissance"]), norm(r["prénoms"]), r["date de naissance"])
            for r in b.rows}
    ...
    if len(b.rows) > 3:
        b.rows[-1]["numéro d'ordre dans le bureau de vote"] = \
            b.rows[-2]["numéro d'ordre dans le bureau de vote"]
    return b.rows
...
def write(rows, path):
    ...
    def field(col, v):
        ...
        return v if col == last else '"%s"' % v

    with open(path, "w", encoding="utf-8", newline="") as f:
        f.write(";".join('"%s"' % c for c in COLUMNS) + "\n")
        for r in rows:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="roll-fixture.csv")
    a = ap.parse_args()
    rows = build(a.rows, a.seed)
    write(rows, a.out)
    print("%d rows written to %s" % (len(rows), a.out))
