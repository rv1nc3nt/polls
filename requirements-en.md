...
| Poll administrator | Modifies the configuration while the poll is in draft; announces, opens, closes and publishes the poll; generates and revokes the draft preview's share link; rules on registrations flagged for manual review. |
...
**R-3.2** State changes are strictly ordered: `draft → announced → open → closed → published`; a poll cannot open without first being announced (R-3.10). From the `announced`, `open`, `closed` or `published` states, and from those alone, a poll may in addition be withdrawn (R-3.11), to a `withdrawn` state that ends its lifecycle. No transition is reversible.
...
**R-3.7** A poll marked as a test poll is excluded from public listings and from all statistics, and its result appears on no public page. This quality is fixed at creation and is not modifiable. A test poll follows the lifecycle of an ordinary poll and can be tried end to end — poll page, registration, voting and, once published, the result — by anyone holding the unguessable link the poll administrator generates for that purpose (rule R-3.10 bis); the ballots cast in it count for nothing. The poll administrator may delete it in any state: the poll, its propositions, its ballots, its registrations and its frozen copy of the electoral roll are then erased. For a test poll, and for it alone, this deletion is an exception to rule R-7.2 (no ballot is ever physically deleted) and to rule R-12.3 (no role deletes a registration); the audit log is never touched, keeps every event of the poll and records the deletion.
...
**R-3.10** A poll still in the `draft` state must be announced before it can open (R-3.2): the poll administrator moves it to the `announced` state, visible on the public site ahead of its opening. The propositions and the calendar are shown there, neither registration nor voting is offered, and the page states expressly that the poll is not yet open. Moving to this state freezes the configuration exactly as moving to the open state does (R-3.3), so the public page cannot change under a viewer's eyes. Announcing is refused if the configuration is missing a translation in an enabled language (R-14.3), just as opening is: the public page cannot freeze, or show, an incomplete configuration. It is also refused once the opening date has already passed: a poll announced after its own opening time would freeze into a public preview only to open immediately behind it, defeating the review this step exists to provide; the administrator must first push the opening date forward, which R-3.3 freely permits while the poll is still in the draft state. A poll still in the draft state appears on no public page.

**R-3.10 bis** Independently of announcing, the poll administrator may generate an unguessable link that lets anyone holding it preview a poll still in the `draft` state, exactly as the administrator's own internal preview shows it — propositions, calendar, neither registration nor voting offered. Unlike announcing, this does not freeze the configuration: the page may change between two visits, exactly as the draft itself may still change, and the page states this plainly. The administrator may regenerate the link at any time, which immediately invalidates the previous one, or revoke it outright; neither requires a reason. The link is never listed or discoverable from the public site, works the same whether or not the poll is marked a test poll (R-3.7) while the poll is still a draft, and once the poll leaves that state it simply redirects to the poll's real public page instead of failing. A test poll is the exception, since it has no public page to redirect to: its link stays valid in every state, unless revoked or regenerated, and then gives access to the poll page, to registration and to voting, on the terms of an ordinary poll (R-3.7).
...
**R-3.12** A poll may hold a small library of images, uploaded once at the poll level and each identified by a short reference shown next to it to the operator. The poll's own description (R-3.1) and, in addition to its label (R-3.1), each option's extended description are formatted text (headings, lists, emphasis, links, and an image drawn from that library by its reference), and may embed video from YouTube — to the exclusion of any other embedded content from an outside source. An option's extended description is optional, and unlike the label's — or the poll's own description's — a missing translation of it is never an obstacle to announcing or opening the poll (R-3.10, R-14.3): an enabled language with no extended description shows the poll's default-language one, or shows nothing at all where that is itself absent. Both descriptions and the image library are frozen with the rest of the configuration at the same moment as the label (R-3.3), and play no part in the tally or in the closure hash (R-10.7).
...
**R-13.3** Retention periods: identity data (registration records, frozen copy of the electoral roll, association of paper ballots with electors) are deleted on expiry of a period of two months running from closure of the poll, or from its withdrawal (R-3.11) where that occurs before closure, and the fields declared under R-12.4 are erased from the audit log at the same term. The starting point is closure, or failing that withdrawal, and not publication: a poll that closes but is never published — an unresolved physical tie-break, an abandoned result — would otherwise keep this data indefinitely, just as a poll withdrawn before closure would otherwise have no starting point at all. Anonymised ballots, the published result where one exists and the log itself are retained beyond that term.
