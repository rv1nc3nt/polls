...
**R-3.7** A poll marked as a test poll is excluded from public listings, from published results and from all statistics. This quality is fixed at creation and is not modifiable. A test poll follows the lifecycle of an ordinary poll and can be tried end to end — poll page, registration, voting — by anyone holding the unguessable link the poll administrator generates for that purpose (rule R-3.10 bis); the ballots cast in it count for nothing. The poll administrator may delete it in any state: the poll, its propositions, its ballots, its registrations and its frozen copy of the electoral roll are then erased. For a test poll, and for it alone, this deletion is an exception to rule R-7.2 (no ballot is ever physically deleted) and to rule R-12.3 (no role deletes a registration); the audit log is never touched, keeps every event of the poll and records the deletion.
