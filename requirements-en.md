...
**R-3.2** State changes are strictly ordered: `draft → [announced] → open → closed → published`. The `announced` state is an optional step (R-3.10): a poll may also go straight from `draft` to `open`. From the `announced`, `open`, `closed` or `published` states, and from those alone, a poll may in addition be withdrawn (R-3.11), to a `withdrawn` state that ends its lifecycle. No transition is reversible.
